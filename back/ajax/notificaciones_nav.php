...
if ($_SESSION["u_nivel"]) {
  ...
  $cantidad_directa =  contar("SELECT count(*) FROM notificaciones WHERE user_2='$user_id' AND visto='0'");
  // todas las notificaciones directas al usuario


/*

  if ($_SESSION["u_nivel"] == '3' || ) {
    $permisos = accessControl();
    if (in_array('go', $permisos)) {
      header("Location: ../../login/logout.php");
    }
    $user_id = $_SESSION["u_ente_id"];
  }*/


  echo $cantidad_directa . '~';


  function vista($id, $u_ente, $tipo, $date, $u_id, $guia, $user_1){

    $contenido = ' <li class="list-group-item list-group-item-action dropdown-notifications-item"  onclick="verNotification(\'' . $id . '\', \'' . $guia . '\', \'' . $user_1 . '\')" >
    <div class="d-flex">
    ...
    $contenido .= '
      ...
      <div class="flex-shrink-0 dropdown-notifications-actions">
        <span class="dropdown-notifications-read"><span class="badge badge-dot"></span></span></div>
    </div>
   </li>';
  ...
  */
  ...
  WHERE user_2='$user_id' AND visto='0' ORDER BY id DESC");
  $stmt->execute();
